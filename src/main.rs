...
static FILE_NAME_REGEX: Lazy<Regex> = lazy_regex!(r#"^attachment; filename="(?P<file_name>.+)"$"#);
static QUERY_REGEX: Lazy<Regex> = lazy_regex!("^page=(?P<page_number>\\d+)$");
static ORIGINAL_NUMBER_REGEX: Lazy<Regex> = lazy_regex!("page/(?P<page_number>\\d+)/?$");
static URL_EXTRACTION_REGEX: Lazy<Regex> =
	lazy_regex!(r#"^<a href="https://www.newgrounds.com/audio/listen/(?P<id>\d+)"#);
static WHITESPACE_REGEX: Lazy<Regex> = lazy_regex!(r"^\s+$");
static PENDING_SCORE_REGEX: Lazy<Regex> = lazy_regex!(r"^\s*Waiting for \d+ more votes?\s*$");
...
pub fn get_files(username: &str) -> anyhow::Result<()> {
	...
	let client = reqwest::blocking::ClientBuilder::new()
		.redirect(Policy::custom(|attempt| {
			...
			let query_page_number_cap = QUERY_REGEX
				.captures(query_string)
				.unwrap()
				.name("page_number")
				.unwrap();
			// let original_number_re = Regex::new("page/(?P<page_number>\\d+)/?$").unwrap();
			let original_url_page_number_cap = ORIGINAL_NUMBER_REGEX
...
pub fn get_metadata_for_ids(ids: &[&str]) -> anyhow::Result<Vec<NewGroundsAudioMetadata>> {
	...
	Ok(ids
		...
		.map(|id| -> anyhow::Result<NewGroundsAudioMetadata> {
			...
			assert!(stats_selector.next().is_none());
											...
											while let Some(child) = sidestats_iter.next() {
												...
												if let Node::Text(text) = value {
													...
													if let Ok(score) = text.parse::<f64>() {
														...
														continue 'main;
													} else if PENDING_SCORE_REGEX.is_match(&text) {
